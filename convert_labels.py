...
def kaggle_xml2edge_impulse(path):
    convertor = KaggleXmlConvertor()
...
def cvat_xml2edge_impulse(path):
    convertor = CVATXmlConvertor()
    parsed = convertor.parse(path)
...
def convert_labels(path, from_format, to_format=LabelFormat.EDGE_IMPULSE):
    if from_format == LabelFormat.CVAT_XML:
        cvat_xml2edge_impulse(args.path)
    elif from_format == LabelFormat.KAGGLE_XML:
        kaggle_xml2edge_impulse(args.path)
    else:
